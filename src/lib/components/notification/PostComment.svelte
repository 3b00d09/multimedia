<script lang="ts">
  ...
  import { onMount } from "svelte";
    ...
    onMount(async()=>{
      if(notification.notification.entityType === "reply") message = "replied to your comment"
      else message = "commented on your post"
    ...
    const navigateToSource = () =>{
        goto(`/post/${data.parentId}/${notification.notification.entityId}`)
    }
...
<button on:click={navigateToSource}>
  <div class="container">
    <div class="user">
      ...
      <p>{data.sourceUser.username + " " + message}</p>
    </div>
